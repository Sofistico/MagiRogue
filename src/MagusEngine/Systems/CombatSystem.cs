    ...
    public static class CombatSystem
    {
        ...
        )
        {
            if (entity is Actor actor)
            {
                ...
                var woundParts = CalculatePartWoundsReceived(
                    attackMomentum,
                    limbAttacked!,
        ...
        private static double CalculateAttackVolume(
            Spell? spellUsed,
            Item? weapon,
        ...
        )
        {
            ...
            {
                double armorEffectiveness = CalculateArmorEffectiveness(
                    targetArmor,
                    attackVolume,
                    attackMaterial,
                    remainingEnergy,
            ...
            while (true) // be realllllllllly careful about this loop...
            {
                ...
                if (!tissues.TryDequeue(out Tissue? tissue))
                {
                    if (partInjured.Insides.Count == 0)
                        break; // even if there is energy, there are no more tissue to penetrate... might do some stuff ltr
                ...
                double energyToPenetrate = CalculateEnergyCostToPenetrateMaterial(
                    tissue.Material,
                    //tissue.Volume,
                    attackMaterial,
                    attack,
                ...
                double woundVolume =
                    attackTotalContactArea <= tissueContactArea
                        ? (double)(
                            tissue.Volume
                            * (double)attackTotalContactArea
                            / (double)tissueContactArea
                        )
                        : tissue.Volume;
        ...
        private static string DetermineDamageMessage(DamageType partDamage, PartWound wound)
        {
            ...
            {
                if (
                    wound.Tissue.Material.ImpactStrainsAtYield <= 24.999
                ...
                )
                {
                    ...
                    wound.Tissue.Material.ImpactStrainsAtYield <= 49.999
                    && wound.Strain >= wound.Tissue.Material.ImpactStrainsAtYield
                )
                {
            ...
            {
                var percent = MathMagi.GetPercentageBasedOnMax(
                    wound.VolumeFraction,
                    wound?.TotalVolume ?? 0
                );
        ...
        public static void ApplyHealing(int dmg, Actor stats, bool healsStamina = false)
        {
            if (healsStamina)
            {
                ...
                if (stats.Body.Stamina >= stats.Body.MaxStamina)
                {
                    ...
                    Locator
                        .GetService<MessageBusService>()
                        .SendMessage<AddMessageLog>(new("You feel your tiredeness fade away"));
                }
            }
            ...
            Locator
                .GetService<MessageBusService>()
                .SendMessage<AddMessageLog>(new($"You healed for {dmg} damage"));
        }
        ...
        private static void ResolveResist(
            MagiEntity poorGuy,
            Actor caster,
            Spell spellCasted,
            ISpellEffect effect,
        ...
        )
        {
            ...
            if (
                effect.IsResistable || Magic.PenetrateResistance(spellCasted, caster, poorGuy, luck)
            )
                DealDamage(
                    effect.BaseDamage,
                    poorGuy,
                    ...
                    attack: attack,
                    spellUsed: spellCasted
                );
            else
                Locator
                    .GetService<MessageBusService>()
                    .SendMessage<AddMessageLog>(
                        new($"{poorGuy.Name} resisted the effects of {spellCasted.Name}")
                    );
        }

        public static void ResolveSpellHit(
            MagiEntity poorGuy,
            Actor caster,
            Spell spellCasted,
            ISpellEffect effect,
        ...
        public static bool ResolveSpellHit(
            MagiEntity poorGuy,
            Actor caster,
            Spell spellCasted,
            ISpellEffect effect
        )
        {
            ...
            else
            {
                ...
                if (
                    poorGuy is Actor actor
                    && diceRoll >= actor.GetDefenseAbility() + Mrn.Exploding2D6Dice
                )
                {
                    return true;
                }
                else
                {
                    Locator
                        .GetService<MessageBusService>()
                        .SendMessage<AddMessageLog>(new($"{caster.Name} missed {poorGuy.Name}!"));
                    return false;
                }
            }
        }
        ...
        )
        {
            ...
            BodyPart bpAttacking =
                attacker.GetAttackingLimb(attack)
            ...
            string with =
                attack?.AttacksUsesLimbName == true
                    ? firstPerson
            ...
            var materialUsed = wieldedItem is null
                ? bpAttacking
                    .Tissues.Find(i =>
                        i.Flags.Contains(TissueFlag.Structural)
                        || i.Flags.Contains(TissueFlag.Muscular)
                    )
                    ?.Material
                : wieldedItem?.Material;
            if (materialUsed is null)
            {
                MagiLog.Log("Material was null!", logLevel: LogLevel.Error);
        ...
        )
        {
            ...
            var initialVelocity = PhysicsSystem.CalculateInitialVelocityFromMassAndForce(
                projectile.Mass,
                force
            );
            ...
            var range = PhysicsSystem.CalculateProjectileRange(
                initialVelocity,
                angle,
                PhysicsConstants.PlanetGravity
            );
            var time = Convert.ToInt64(
                PhysicsSystem.CalculateProjectileTime(
                    initialVelocity,
                    angle,
                    PhysicsConstants.PlanetGravity
                )
            ...
            var projectileComp = new ProjectileItemComp(
                time,
                origin,
                pointToGo,
                direction,
        ...
        public static void HitProjectile(
            MagiEntity? projectile,
            Point lastPoint,
            DamageType dmg,
            Material material,
            double force,
            bool ignoresObstacles
        )
        {
            var (sucess, entity, tile) = GetWhatToHitProjectile(
                projectile,
                lastPoint,
                ignoresObstacles
            );
            ...
            if (tile != null)
            {
                ...
                if (
                    tile.Material.ImpactStrainsAtYield >= projectile.GetMaterial().ImpactFractureMpa
                )
                {
        ...
        public static void HitSpellProjectile(
            [NotNull] MagiEntity projectile,
            ISpell spell,
            Point lastPoint,
            bool ignoresObstacles
        )
        ...
        )
        {
            ...
            var point = projectile.Position;
            var map = projectile.CurrentMagiMap;
            var entities = map.GetEntitiesAt<MagiEntity>(
                point,
                map.LayerMasker.Mask(
                    (int)MapLayer.ACTORS,
                    (int)MapLayer.ITEMS,
        ...
        public static void ResolveDeath(Actor defender)
        {
            ...
            if (defender.Inventory.Count > 0)
            {
                foreach (Item item in defender.Inventory)
                {
                    ...
                    Locator
                        .GetService<MessageBusService>()
                        .SendMessage<AddEntitiyCurrentMap>(new(item));
                }
            ...
            Locator
                .GetService<MessageBusService>()
                .SendMessage<RemoveEntitiyCurrentMap>(new(defender));

            if (defender is Player)
            {
                Locator
                    .GetService<MessageBusService>()
                    .SendMessage<AddMessageLog>(new($" {defender.Name} was killed."));
            }

            // Now show the deathMessage in the messagelog
            Locator
                .GetService<MessageBusService>()
                .SendMessage<AddMessageLog>(new(deathMessage.ToString()));
        }
            ...
            if (percentileRoll >= armor.Coverage)
            {
                switch (armor.ArmorType)
                {
                    ...
                    case ArmorType.Chain:
                        ...
                        momentumAfterArmor = CalculateEnergyCostToPenetrateMaterial(
                            armor.Material!,
                            //armor.Volume,
                            attackMaterial,
                            attack,
        ...
        private static double CalculateEnergyCostToPenetrateMaterial(
        ...
        )
        {
            return attack.DamageType?.Type switch
            {
                ...
                DamageTypes.Pierce
                or DamageTypes.Sharp
                    => CalculateEdgedDefenseCost(
                        defenseMaterial,
                        attackMaterial,
                        //defenseMaterialVolume,
                        attack.ContactArea,
        ...
        public static double CalculateEdgedDefenseCost(
            Material defenseMaterial,
            Material attackMaterial,
            //double layerVolume,
        ...
        public static double CalculateBluntDefenseCost(
            Material defenseMaterial,
        ...
        {
            var bluntDeflection =
                2 * attackSize * defenseMaterial.ImpactYield
            ...
            {
                var minimumMomentum =
                    ((2 * defenseMaterial.ImpactFracture) - defenseMaterial.ImpactYield)
                    * (2 + (0.4 * armorQualityMultiplier))
        ...
        public static double GetAttackMomentumWithItem(
            Actor attacker,
            Item? wieldedItem,
            Attack attack
        )
        {
            return (
                ...
                + (
                    ...
                    + (
                        attacker.Volume
                        / ((wieldedItem?.Material?.DensityKgM3 ?? 1) * wieldedItem?.Volume ?? 1)
                    )
                );
        }
